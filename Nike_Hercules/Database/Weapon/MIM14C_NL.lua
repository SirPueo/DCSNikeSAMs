...
local MIM14C_NL = {
	name = MIM14C_NL,
	display_name = _('MIM-14C Nike Hercules (NL)'),
	display_name_short = _('MIM-14C (NL)'),
	name = "MIM14C_NL",
	...
	Life_Time = 240.0,
	Nr_max = 10, -- Maximum g when turning 7 for -14B, 10 -14C
	v_min = 0.0,
	v_mid = 900.0,
	Mach_max = 3.5,
	...
	ModelData = {
		...
		--t_start   t_boost     t_accel    t_march     t_inertial   t_break  t_end   --Stage
		 -1.0,	    3.4,	    5.6,	   29,		    0.0,	    0.0,	 1.0e9,  -- time of stage, sec
		 0.0,	    705.8,	    0.01,	   11.7,		0.0,	    0.0,	 0.0,    -- fuel flow rate, kg/sec
		 0.0,	    800000.0,	10,	       80000.0,	    0.0,	    0.0,	 0.0,    -- thrust, newtons
...
declare_weapon(MIM14C_NL)

GT_t.WS_t.NikeHerculesC_NL = {}
GT_t.WS_t.NikeHerculesC_NL.moveable = false
GT_t.WS_t.NikeHerculesC_NL.angles = {
					{math.rad(180), math.rad(-180), math.rad(-90), math.rad(90)},
					};

GT_t.WS_t.NikeHerculesC_NL.LN = {}
GT_t.WS_t.NikeHerculesC_NL.LN[1] = {}
GT_t.WS_t.NikeHerculesC_NL.LN[1].type = 4
GT_t.WS_t.NikeHerculesC_NL.LN[1].distanceMin = 6500;
GT_t.WS_t.NikeHerculesC_NL.LN[1].distanceMax = 155000;
GT_t.WS_t.NikeHerculesC_NL.LN[1].ECM_K = 0.3; -- Notional susceptibility to chaff and ECM (mostly handled through other mechanics, but a -1 means no effect, often seen with IR missiles). 
GT_t.WS_t.NikeHerculesC_NL.LN[1].reactionTime = 4; --Acquisition time for the weapon sighting system (should correspond to that of the the carrying vehicle but may be overwritten depending on sensors and data-linking capabilities).
GT_t.WS_t.NikeHerculesC_NL.LN[1].launch_delay = 6; --Time in seconds between successive missile launches (time to ready new missile). 
GT_t.WS_t.NikeHerculesC_NL.LN[1].reflection_limit = 0.02;
GT_t.WS_t.NikeHerculesC_NL.LN[1].show_external_missile = true;
GT_t.WS_t.NikeHerculesC_NL.LN[1].sensor = {}
set_recursive_metatable(GT_t.WS_t.NikeHerculesC_NL.LN[1].sensor, GT_t.WSN_t[0])
GT_t.WS_t.NikeHerculesC_NL.LN[1].beamWidth = math.rad(1);
GT_t.WS_t.NikeHerculesC_NL.LN[1].PL = {}
GT_t.WS_t.NikeHerculesC_NL.LN[1].PL[1] = {}
GT_t.WS_t.NikeHerculesC_NL.LN[1].PL[1].ammo_capacity = 1;
GT_t.WS_t.NikeHerculesC_NL.LN[1].PL[1].type_ammunition = MIM14C_NL.wsTypeOfWeapon;
GT_t.WS_t.NikeHerculesC_NL.LN[1].PL[1].reload_time = 15*60; -- 15 minutes
GT_t.WS_t.NikeHerculesC_NL.LN[1].BR = { {pos = {0, 0, 0} } }
